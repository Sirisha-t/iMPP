...
workflow
{
  ...
  if (params.single == "null"){ 
      if(params.interleaved == "null"){
	    ...
	    if( (forwardfile.getExtension() ==  "fq" || forwardfile.getExtension() ==  "fastq"|| forwardfile.getExtension() == "gz") && (reversefile.getExtension() ==  "fq" || reversefile.getExtension() ==  "fastq" || reversefile.getExtension() == "gz")){
              ...
              inputfile = Channel.fromPath(params.forward)
  ...
  if(params.genecaller == "fgs")
    ...
    peptideAssembly_first(sixFrameTranslate.out.filtered_reads)
    modifyAssemblyOut(peptideAssembly_first.out.plass_first_round)
    ...
    peptideAssembly_second(refinePlassInput.out.impp_filtered_reads)
    
    cleanOutput(callFGS.out.reads_gff, callFGS.out.reads_ffn, callFGS.out.reads_faa, callFGSPaths.out.fgs_path_gff, callFGSPaths.out.fgs_path_ffn, callFGSPaths.out.fgs_path_faa, peptideAssembly_second.out.final_plass_assembly)    


   //result = params.intermediate.deleteDir()
   //println result ? "Deleted intermdiate files" : "Cannot delete: $params.intermdiate"
...
process cleanOutput{
  ...
  path 'assembled_proteins.faa', emit: assembly_final_output
  script:
  """
